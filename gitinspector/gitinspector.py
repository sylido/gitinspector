...
class Runner():
  ...
  def process(self, repos):
    ...
    f = open("gitinspectorStats.html", "w")

    # format.output_header(repos)
    f.write(format.output_header(repos))


    # outputable.output(ChangesOutput(summed_changes))
    f.write(outputable.output(ChangesOutput(summed_changes)))

    if summed_changes.get_commits():
      # outputable.output(BlameOutput(summed_changes, summed_blames))
      f.write(outputable.output(BlameOutput(summed_changes, summed_blames)))

      if self.timeline:
        # outputable.output(TimelineOutput(summed_changes, self.useweeks))
        f.write(outputable.output(TimelineOutput(summed_changes, self.useweeks)))

      if self.include_metrics:
        # outputable.output(MetricsOutput(summed_metrics))
        f.write(outputable.output(MetricsOutput(summed_metrics)))

      if self.responsibilities:
        # outputable.output(ResponsibilitiesOutput(summed_changes, summed_blames))
        f.write(outputable.output(ResponsibilitiesOutput(summed_changes, summed_blames)))

      # outputable.output(FilteringOutput())
      f.write(outputable.output(FilteringOutput()))

      if self.list_file_types:
        # outputable.output(ExtensionsOutput())
        f.write(outputable.output(ExtensionsOutput()))

    # format.output_footer()
    f.write(format.output_footer())
    f.close()
