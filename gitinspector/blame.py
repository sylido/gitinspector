...
# NUM_THREADS        = 1
print("Running on %d thread(s)" % (NUM_THREADS))
...
class BlameThread(threading.Thread):
  ...
  def __handle_blamechunk_content__(self, content):
    ...
    if (
      ...
      try:
          some_iter = iter(self.changes)
      except TypeError as te:
        # print("self.changes is not iterable")
        __blame_lock__.release()  # ...to here.
        pass
      else:
        if ("first_commit_date" in self.changes):
          self.changes.first_commit_date = self.changes.first_commit_date
        else:
          self.changes.first_commit_date = datetime.datetime.now()
