...
def output_header(repos):
  ...
  if __selected_format__ == "html" or __selected_format__ == "htmlembedded":
    ...
    #   "latin-1", "replace"
    # )
    tablesorter_js = __get_zip_file_content__("tablesorter-master/dist/js/jquery.tablesorter.min.js", "/html/tablesorter-master.zip").encode(
      "latin-1", "replace"
    )
    ...
    if __selected_format__ == "htmlembedded":
      ...
      jquery_js = ' src="https://ajax.googleapis.com/ajax/libs/jquery/3.7.1/jquery.min.js">'

    res = html_header.format(
      ...
      repo_text          = _(INFO_ONE_REPOSITORY if len(repos) <= 1 else INFO_MANY_REPOSITORIES).format(
        repos_string, datetime.datetime.now()
      ),
    ...
    res += '\t\t"version": "' + version.__version__ + '",'

    if len(repos) <= 1:
      # print('\t\t"repository": "' + repos_string + '",')
      res += '\t\t"repository": "' + repos_string + '",'
    else:
    ...
    res += '\t\t"report_date": "' + time.strftime("%Y/%m/%d") + '",'
    ...
    res += "\t<version>" + version.__version__ + "</version>"

    if len(repos) <= 1:
      # print("\t<repository>" + repos_string + "</repository>")
      res += "\t<repository>" + repos_string + "</repository>"
    else:
      # print("\t<repositories>")
      res += "\t<repositories>"
      ...
      for repo in repos:
        ...
        res += "\t\t<repository>" + repo.name + "</repository>"

      # print("\t</repositories>")
      res += "\t</repositories>"

    # print("\t<report-date>" + time.strftime("%Y/%m/%d") + "</report-date>")
    res += "\t<report-date>" + time.strftime("%Y/%m/%d") + "</report-date>"
    return res

  else:
    res = textwrap.fill(
      _(INFO_ONE_REPOSITORY if len(repos) <= 1 else INFO_MANY_REPOSITORIES).format(repos_string, datetime.datetime.now()),
